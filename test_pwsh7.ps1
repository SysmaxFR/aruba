...
#$Vlans = Invoke-RestMethod -Uri "http://$IP/$Rest/vlans" -Method GET -ContentType 'application/json' -WebSession $Cookie
...
$StatusVsfMember1 = (($VsfMember1 -split "`n") | ? {$_ -match "status"}) | ConvertFrom-StringData -Delimiter ":"  #Convert the String to an array of string, then filter
